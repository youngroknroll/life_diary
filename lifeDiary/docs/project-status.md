...
| Pytest translation compilation | `docs/plans/2026-05-25-pytest-compilemessages.md`, `docs/refactoring/2026-05-25_pytest-compilemessages.md` | Added a pytest startup hook that compiles Korean and English Django message catalogs before tests run, preventing English locale regressions from depending on pre-existing generated `.mo` files. | Fresh verification: RED `pytest test_pytest_i18n_compile.py --tb=short` -> missing helper import; GREEN same command -> `1 passed in 0.03s`; focused English locale regression -> `1 passed in 0.87s`; full suite -> `226 passed in 110.00s`. |
