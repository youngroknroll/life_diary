...
# → 2026-08-17 목표 진행 바(build_goal_progress_rows)가 UserGoal 목록 조회
#   1회를 더한다(18). 이 항목은 앞의 것들과 달리 기록량이 아니라 "목표 개수"에
#   비례한다 — 목표마다 그 기간의 TimeBlock 합을 한 번씩 더 조회한다. 이
#   fixture는 목표를 만들지 않아 그 N+1은 여기서 드러나지 않는다. 온보딩이
#   "하나면 충분하다"고 안내해 실사용 목표 수가 작을 것으로 보고 지금은
#   허용한다 — 목표가 여러 개인 사용자가 흔해지면 이 상수도 목표 수만큼
#   묶어 세는 방식으로 재검토한다.
TARGET_MAX_QUERIES = 18
